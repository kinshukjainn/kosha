...
function FormatBadge({ ext }: { ext: string }) {
  return (
    <span className="inline-block items-center px-2.5 py-1 bg-blue-300 dark:bg-[#121212] text-black dark:text-blue-300  text-[13px] font-bold rounded-2xl  mr-2 mb-2 select-none">
      {ext}
    </span>
  );
}
...
function SectionAnchor({ id, children }: { id: string; children: ReactNode }) {
  return (
    <a
      href={`#${id}`}
      className="group relative text-gray-900 dark:text-gray-100 hover:text-blue-800 dark:hover:text-blue-300 transition-colors duration-150"
    >
      {children}
    </a>
  );
...
function SystemNotice({ children }: { children: ReactNode }) {
  return (
    <div className="my-6 p-4 border-l-4 border-blue-300 dark:bg-[#1e1e1e] bg-gray-200 text-gray-900 dark:text-gray-100 flex gap-3 items-start rounded-r-2xl">
      <AlertTriangle className="w-5 h-5 dark:text-gray-200 text-gray-900 shrink-0 mt-0.5" />
...
export default function SupportedFormatsPage() {
  return (
    <div className="min-h-screen text-gray-100 dark:text-gray-900 pb-24 bg-white dark:bg-black  selection:bg-[#cce3f5]">
      {/* Header / Command Bar */}
      ...
      <main className="max-w-[1000px] mx-auto px-6 pt-12">
          ...
          <ul className="grid sm:grid-cols-2 gap-y-2 gap-x-8 text-[13px] list-none p-0 m-0">
            {TOC_ITEMS.map((item) => (
              <li key={item.id}>
                <a
                  href={`#${item.id}`}
                  className="dark:text-blue-300 text-blue-900 hover:underline transition-all duration-150"
                >
                  {item.label}
                </a>
              </li>
        ...
        <section id="images" className="mb-16 scroll-mt-20">
          ...
          <div className="flex flex-wrap bg-gray-200 dark:bg-[#141414] p-5  rounded-2xl rounded-2xl shadow-sm">
        ...
        <section id="videos" className="mb-16 scroll-mt-20">
          ...
          <div className="flex flex-wrap bg-gray-200 dark:bg-[#141414] p-5  rounded-2xl shadow-sm">
        ...
        <section id="archives" className="mb-16 scroll-mt-20">
          ...
          <div className="flex flex-wrap bg-gray-200 dark:bg-[#141414] p-5  rounded-2xl rounded-2xl shadow-sm mb-6">
